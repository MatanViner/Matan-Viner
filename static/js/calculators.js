...
currentUser.measures = measurements;
const customer = new Customer(currentUser);
...
calculatorButton.onclick = function (e) {
  ...
  bodyFatInput.value =
    getBodyFat(measurements.at(-1), customer.getAge(), customer.gender).toFixed(
      2
    ) + "%";
  bmrInput.value = customer.getCurrentBMR().toFixed(2);
  const BMI = getBMI(measurements.at(-1)).toFixed(2);
  bmiInput.value = BMI;
  updateBmiSlider(BMI);
  ...
  if (!menuTable.innerHTML.includes(menus[stage])) {
